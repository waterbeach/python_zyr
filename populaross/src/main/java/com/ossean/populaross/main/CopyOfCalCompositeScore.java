...
@Component("copyOfCalCompositeScore")
public class CopyOfCalCompositeScore {
	...
	private static String sourceTableName = "osp_test_thread";
	private static String targetTableName = "osp_test_thread";
...
//logger.info("memo_id : "+relativeMemoId);
			...
			monthNum = 2;//GetTime.getMonth(rmToOsp.getCreated_time(),rmToOspDao.getCrawledTime(relativeMemoId, relativeMemosTableName));
...
//		+"cost time: "+(System.currentTimeMillis()-startTime)/60000+" minutes");
	...
	public static void main(String [] args){
		...
		CopyOfCalCompositeScore main = applicationContext.getBean(CopyOfCalCompositeScore.class);
		main.start();
	    }
	...
	public void start(){
		...
		while(true){
			...
			if(startId > maxId){
				try {
					System.out.println("finally cost time: "+(System.currentTimeMillis()-startTime)/1000+"seconds");
